...
async def fetch(conn: AsyncConnection, name: str, params: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    result = await run_query(conn, bubble_query(name), params)
    return [dict(row) for row in result.rows]


async def test_every_active_bubble_meets_min_candidates(db_connection: AsyncConnection) -> None:
    """후보가 min_candidates 미만인 버블은 화면에 내면 안 됩니다.

    비어 있는 버블을 누르게 하는 것이 가장 나쁜 경험입니다.
    """
    rows = await fetch(db_connection, "bubble_candidate_counts", {})
...
async def test_bubble_candidates_return_recipes(db_connection: AsyncConnection) -> None:
    """버블마다 실제로 레시피가 나와야 합니다."""
    for row in await fetch(db_connection, "bubble_candidate_counts", {}):
        recipes = await fetch(
            db_connection, "bubble_recipe_candidates", {"keyword_id": row["keyword_id"], "max_results": 5}
        )
        assert recipes, f"{row['keyword_id']} 버블에서 레시피가 나오지 않습니다"
        assert all(item["name"] for item in recipes)


async def test_low_ingredient_bubble_respects_its_threshold(db_connection: AsyncConnection) -> None:
    """`재료 적게 드는 요리` 는 임계값을 실제로 지켜야 합니다."""
    recipes = await fetch(
        db_connection, "bubble_recipe_candidates", {"keyword_id": "LOW_INGREDIENT", "max_results": 30}
    )
...
async def test_quick_bubble_respects_cook_time(db_connection: AsyncConnection) -> None:
    """`15분 안에 끝나는 요리` 에 조리시간이 없는 레시피가 섞이면 안 됩니다."""
    recipes = await fetch(db_connection, "bubble_recipe_candidates", {"keyword_id": "QUICK_15MIN", "max_results": 30})

    assert recipes
    assert all(item["cook_time_min"] is not None and item["cook_time_min"] <= 15 for item in recipes)


async def test_meat_bubble_excludes_egg_only_desserts(db_connection: AsyncConnection) -> None:
    """`고기 든든하게` 규칙에 `난류` 를 넣었더니 `레몬 커드` 가 걸렸습니다.

    육류만 보도록 고쳤고, 그 결정을 여기서 고정합니다.
    """
    recipes = await fetch(db_connection, "bubble_recipe_candidates", {"keyword_id": "MEAT", "max_results": 50})
