...
| 항목 | 적재 쪽 할 일 |
| --- | --- |
| 영문 레시피 전량 제외 | `recipe.source_type = 'recipes'` 1,004건. **결정됨 — 지우지 않습니다.** 이미 적재된 1,004건은 **보존**하고, **신규 적재만** 한국어 raw 로 제한합니다. FK 가 걸려 있어 삭제는 비싸고, `data/raw/` 에 영문 parquet 을 넣지 않으면 코드 변경도 필요 없습니다 |
