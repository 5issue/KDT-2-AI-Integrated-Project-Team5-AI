...
### 먼저: 영문 레시피 제외가 결정되면 버블 4개가 못 뜹니다

디자인팀이 이번 스프린트에서 영문 레시피를 전량 제외하기로 했습니다.
적재분 1,086건 중 **1,004건이 영문 원천(allrecipes)** 이고, 한국어 원천은 30건뿐입니다.
...
| `QUICK_15MIN` | 233 | **0** |
| `LIGHT` | 259 | **2** |
| `LOW_INGREDIENT` | 679 | **8** |
| `MEAT` | 159 | 15 |

한국어 원천은 `cook_time_min` 이 0건이고 평균 조리 단계가 26.3 입니다. 단계 수·조리시간
규칙에 구조적으로 안 걸립니다. **`min_candidates=10` 을 넘는 것은 MEAT 하나뿐입니다.**
